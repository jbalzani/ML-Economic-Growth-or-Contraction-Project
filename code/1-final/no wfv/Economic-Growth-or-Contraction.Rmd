...
Data from 1982 are used to make lags of variables and the first differences of the variables, data from 1983 to April 2008 are used to estimate the models used, and data from July 2008 to October 2019 are used to evaluate the models. The economic growth data consists of quarterly observations of real US GDP (economic) growth from January 1982 to October 2019. The CLI data consists of monthly observations of the Composite Leading Index from June of 1961 to December of 2019.

This project undertook the following key steps. First, the data were cleaned and explored. Second, the data are tested for stationarity, since they are time series data. Stationarity means that the "statistical properties are constant over time", and this is required to do much statistical analysis (Nau, 2019). Third, the models were created. Fourth, the models were evaluated with accuracy and F1 score used as the metrics to judge model effectiveness. It is found that the logistic regression is the best out of these models for predicting economic growth or contraction. This model has an accuracy of 93.5% during the test period, which is quite good, and a F1 score of .727. This model outperforms a simple strategy of predicting economic growth for every period, which would give an accuracy of 84.8% for the test period. The other model results were the regularized logistic model (accuracy 84.8%, F1 score 0), random forest model (accuracy 84.8%, F1 score 0), the ensemble model (accuracy 84.8%, F1 score 0), and the knn model (accuracy 84.8%, F1 score 0).
...
Conclusion: ADF test should be repeated with larger lag length, as the absolute value of the t statistic of the last lagged value is greater than 1.6, so a maximum lag length for the ADF test has not yet been found.
...
```{r adf test CLI 8 lags, include=TRUE}
#adf test cli 8 lags
...
In order to explore the number of lags to test for real GDP growth, a correlogram of the  ACFs was made (Figure 6). It can be seen that the 1-4 lagged values of real GDP growth are correlated with its current level.


```{r acf gdp}
acfs_gdp <- acf(combined_data_train_ts[, 'real_gdp_growth'], lag.max = 12, na.action = na.pass, main = "Figure 6: ACFs of Lags of Real GDP Growth")
```

A correlogram of the PACF for real GDP growth was also generated (Figure 7). It can be seen that the first and second lags are correlated with its current level. The 11th lag is also significant, but this may be an artifact of the data rather than real causation because none of the other lags after lag 2 are significant.

```{r pacfs gdp}
pacfs_gdp <- pacf(combined_data_train_ts[, 'real_gdp_growth'], lag.max = 12, na.action = na.pass, main = "Figure 7: PACFs of Lags of Real GDP Growth")
```

In order to explore the number of lags to test for the CLI, correlograms of the ACFs and PACFs were made (Figure 8, Figure 9). From the ACF correlogram, it can be seen that the 1-4 lagged values of the CLI are correlated with its current level. From the PACF correlogram, we can see that only the once and twice lagged values are signficant.

```{r acfs cli}
acfs_cli <- acf(combined_data_train_ts[, 'CLI'], lag.max = 12, na.action = na.pass, 
                main = "Figure 8: ACFs of Lags of CLI")
```

```{r pacfs cli}
pacfs_cli <- pacf(combined_data_train_ts[, 'CLI'], lag.max = 12, na.action = na.pass,
                  main = "Figure 9: PACFs of Lags of CLI")
```


Next, just the CLI time series is plotted, in order to get a better visual on the series and to see if there might be seasonality (Figure 10). As the amplitude-adjusted CLI is calculated from smoothed component series, there shouldn't be seasonality, however, it can be a good exercise to check. A plot of the CLI by quarter is also plotted to see if there is quarterly seasonality (Figure 11). As can be seen from the plot, there is no clear seasonality in the data.

```{r cli plot}
combined_data_train %>% ggplot(aes(x = DATE, y = CLI)) +
  geom_line() +
  ggtitle("Figure 10: CLI")
```
...
ts_cli <- ts(combined_data_train$CLI, 
             start = min(year(combined_data_train$DATE)),
              ...
              frequency = 4)
ggseasonplot(ts_cli, year.labels = TRUE, continuous = TRUE, main = "Figure 11: Seasonal Plot of CLI")
```
Model Creation:
\newline
...
```{r reg logit no wf validation, include=FALSE}
k <- 0
...
```{r wf validation reg logit, include = FALSE}
...
```{r cross validation regularized logit model, include = FALSE}
...
A knn algorithm is then created to model economic growth or contraction, and this model is next optimized for the best value of k. Values between 3 and 25 were tested, and a value of 3 was found to be the optimal k. Finally, a random forest model is created. In order to get an idea of how many trees to have in my random forest model, I generated a plot of the model, from which we can see that the algorithm converges after no more than 100 trees (Figure 12). The random forest algorithm with 100 trees is then optimized for the best value of mtry. Mtry values between 1 and 6 were tested, and the optimal value was found to be 1.
...
plot(rf_model_initial, main = "Figure 12: Initial RF Model Error vs. Number of Trees")
```
...
**Results:**
After testing the performance against the test set for each model, we can see that logistic regression is the best-performing model, with an accuracy of 93.5% and a F1 score of 0.727. This model had perfect specificity, while having the best sensitivity at 0.57, with the positive class being a "0", or economic contraction for all models.

The other models had an accuracy of 84.8% and an F1 score of 0, having predicted economic growth for all time periods. Thse models had a sensitivity of 0., so there is room for improvement.
...
test_nml <- combined_data_test %>%  
  filter(real_gdp_growth_lag_nml > 1 | real_gdp_growth_lag_nml < 0 |
        real_gdp_growth_lag2_nml > 1 | real_gdp_growth_lag2_nml < 0 |
        real_gdp_growth_lag3_nml > 1 | real_gdp_growth_lag3_nml < 0 |
...
```{r confusion matrix regularized logit model}
confusionMatrix(factor(y_hat_reglogit), reference = factor(combined_data_test$gdp_impr))
#calculate f1 score - code won't run since score is 0, because no correct positive predictions
#f1_reg_logit <- F_meas(factor(y_hat_knn), factor(combined_data_test$gdp_impr))
```
...
combined_data_test %>% ggplot(aes(x = DATE)) +
  ...
  ggtitle("Figure 13: Logit Model Predictions vs. Real Economic Outcomes", subtitle = "Red Indicates a Miss")
```
\newpage
Regularized Logistic Regression (Figure 14): \newline
```{r plot regularized logistic regression}
combined_data_test %>% ggplot(aes(x = DATE)) +
  geom_point(aes(y = gdp_impr, color = "gdp_impr")) +
  geom_point(aes(y = reg_logit, color = "reg_logit")) +
  ggtitle("Figure 14: Regularized Logit Model Predictions vs. Real Economic Outcomes", 
...
combined_data_test %>% ggplot(aes(x = DATE)) +
  ...
  ggtitle("Figure 15: KNN Model Predictions vs. Real Economic Outcomes", subtitle = "Red Indicates a Miss")
```
\newpage
...
combined_data_test %>% ggplot(aes(x = DATE)) +
  ...
  ggtitle("Figure 16: Random Forest Model Predictions vs. Real Economic Outcomes", subtitle = "Red Indicates a Miss")
```
\newpage
Ensemble (Figure 17):
\newline
...
combined_data_test %>% ggplot(aes(x = DATE)) +
  ...
  ggtitle("Figure 17: Ensemble Model Predictions vs. Real Economic Outcomes", subtitle = "Red Indicates a Miss")
```

```{r mean of gdp_impr}
#calculate mean gdp_impr for baseline comparison
mean_gdp_impr <- combined_data_test %>%
  mutate(gdp_impr = as.numeric(gdp_impr)) %>%
  mutate(gdp_impr = ifelse(gdp_impr == 2, 1, 0)) %>%
  .$gdp_impr %>%
  mean()
```

```{r comparison to previous prediction, include=FALSE}
since_original <- combined_data_test %>%
  filter(DATE > '2019-07-01')
since_original_acc <- mean(since_original$logit == since_original$gdp_impr)
```


**Conclusion:**

Models based on the lagged values of CLI and real GDP growth were developed using the training portion of the  dataset, and these were tested on the test set. The best-performing model was found to have an accuracy of `r round(100*mean(y_hat_logistic_all == combined_data_test$gdp_impr), 3)` % and an F1 score of `r round(f1_logit, 3)`. This indicates that the model did a good job of predicting whether or not there would be economic growth from July 2008 - October 2019. This is better than a strategy of simply assuming that there will be economic growth in every period, which would yield an accuracy of `r round(100*mean_gdp_impr, 3)`%.

While the model accuracies are acceptable, there are limitations to these models. The logistic regression model did not converge when lag 2 of real GDP growth was included in the model, and while the logistic regression still performed very well, it would be interesting to think about what the results might be if this variable had been included. The regularized logistic regression, the random forest model, and the knn model failed to predict any periods of contraction, so they were not as sensitive or accurate as I hoped. The F1 scores of these models were zero, so there is further research to be done with these models on a larger data set.
\newpage
**References:**
...
Friedman, Jerome, Trevor Hastie, and Robert Tibshirani. “Reguarlization Paths for Generalized Linear Models via Coordinate Descent.” Journal of Statistical Software 33, no. 1 (2010): 1–22.
...
Hyndman, R, G Athanasopoulos, C Bergmeir, G Caceres, L Chhay, M O’Hara-Wild, F Petropoulos, S Razbash, E Wang, and F Yasmeen. _forecast: Forecasting Functions for Time Series and Linear Models_ (version R package version 8.11), 2020. http://pkg.robjhyndman.com/forecast.
Hyndman, RJ, and Y Khandakar. “Automatic Time Series Forecasting: The Forecast Package for R.” Journal of Statistical Software 26, no. 3 (2008): 1–22.
...
James, Gareth, Daniela Witten, Trevor Hastie, and Robert Tibshirani. An Introduction to Statistical Learning. 7th ed. Springer, 2013.

Kuhn, Max. Caret: Classification and Regression Training (version R package version 6.0-85), 2020. https://CRAN.R-project.org/package=caret.

Liaw, A, and M Weiner. “Classification and Regression by Random Forest.” R News, 2002.
Meyer, David, Evgenia Dimitriadou, Kurt Hornik, Andreas Weingessel, and Friedrich Leisch. E1071: Misc Functions of the Department of Statistics, Probability Theory Group (Formerly: E1071) (version R package version 1.7-3), 2019. https://CRAN.R-project.org/package=e1071.

Nau, Robert. “Stationarity and Differencing,” 2019.
...
Organization for Economic Co-operation and Development. “Leading Indicators OECD: Leading Indicators: CLI: Amplitude-Adjusted for OECD - Total  [OECDLOLITOAASTSAM].” https://fred.stlouisfed.org/series/OECDLOLITOAASTSAM, n.d. Accessed February 19, 2020.

Peduzzi et al., Peter. “A Simulation Study of the Number of Events per Variable in Logistic Regression Analysis.” Journal of Clinical Epidemiology 49, no. 12 (n.d.): 1373–79.
...
Shao, L, X Fan, N Cheng, L Wu, and Y Cheng. “Determination of Minimum Training Sample Size for Microarray-Based Cancer Outcome Prediction-An Empirical Assessment.” PLoS ONE 8, no. 7 (2013). https://doi.org/10.1371/journal.pone.0068579.
